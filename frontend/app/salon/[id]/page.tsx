...
export default function SalonHome() {
  const params = useParams(); // 👈 Capture Salon ID
  const router = useRouter(); // 👈 Navigation
  const salonId = params.id;  // (We will use this later to fetch specific salon data)
    ...
    <main className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-black to-black flex items-center justify-center p-4 md:p-8 font-sans text-gray-200 overflow-hidden">
      ...
      <div className="w-full max-w-7xl grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10">
          ...
          <div className="hidden lg:flex flex-col space-y-8 animate-in slide-in-from-left-10 duration-1000 fade-in">
              {/* Added Back Button for Desktop */}
              <button onClick={() => router.push('/')} className="self-start flex items-center gap-2 text-gray-400 hover:text-white transition-colors group mb-4">
                  <span className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center border border-white/10 group-hover:border-green-500/50 group-hover:text-green-400 transition-all">←</span>
                  <span className="text-sm font-medium uppercase tracking-wider">Back to Salons</span>
              </button>
              ...
              <h1 className="text-7xl font-black text-transparent bg-clip-text bg-gradient-to-r from-green-400 via-emerald-500 to-teal-600 tracking-tight animate-in slide-in-from-bottom-5 duration-1000 delay-100">
                  {/* Dynamic Salon Name (Mocked for now) */}
                  {salonId === "salon_101" ? "Cool Cuts Mumbai" : "SlotSync Salon"}
              </h1>
              <p className="text-2xl text-gray-400 max-w-lg leading-relaxed font-light animate-in slide-in-from-bottom-5 duration-1000 delay-200">
                  You are viewing the live queue for <span className="text-white font-medium">{salonId}</span>. <br/>
          ...
          <div className="flex flex-col items-center lg:items-end w-full">
              ...
              <div className="w-full max-w-md flex justify-between items-center mb-6 px-2 animate-in fade-in slide-in-from-top-5 duration-700">
                  ...
                  <div className="lg:hidden flex items-center gap-3">
                      <button onClick={() => router.push('/')} className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-white border border-white/10 active:scale-95 transition-transform">←</button>
                      <h1 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-500 truncate max-w-[150px]">
                        {salonId === "salon_101" ? "Cool Cuts" : "SlotSync"}
                      </h1>
                  </div>
              ...
              {/* Main Card - PRESERVED EXACTLY AS BEFORE */}
