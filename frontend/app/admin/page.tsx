...
export default function Admin() {
  ...
  const [salonId, setSalonId] = useState("salon_101"); // 👈 NEW: Default to salon_101
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (res.ok) { 
            setIsLocked(false); 
            refresh(); // Load data for the chosen salonId immediately
        } 
  ...
  }, [isLocked, salonId]); // Refresh if salonId changes

  const refresh = async () => {
    try {
        // 👇 NEW: Send salon_id to get specific queue
  ...
  const apiCall = async (endpoint: string, body: any) => {
      // 👇 NEW: Always attach salon_id to actions so backend knows which queue to update
          ...
          <div className="space-y-4">
            ...
            {data?.queue.map((p: any, index: number) => (
                <div key={p.token} onClick={() => setSelectedUser(p)} 
                    ...
                    ${index===0 ? 'bg-gradient-to-r from-blue-900/20 to-blue-900/10 border-blue-500/50 shadow-[0_0_30px_rgba(59,130,246,0.1)]' : 'bg-neutral-900/50 border-white/5 hover:bg-neutral-800 hover:border-white/10'}`}>
                    ...
                    <div className="flex gap-2 w-full md:w-auto justify-end border-t border-white/5 pt-4 md:pt-0 md:border-t-0">
                        <button onClick={(e) => { e.stopPropagation(); setEditingUser(p); setEditServices(p.services); }} className="p-3 bg-neutral-800 hover:bg-white hover:text-black text-gray-300 rounded-xl transition-colors">✎</button>
                        ...
                        {index !== 0 && <button onClick={(e) => serveNow(p.token, e)} className="p-3 bg-yellow-500/10 text-yellow-500 hover:bg-yellow-500 hover:text-black rounded-xl transition-colors">⚡</button>}
                        <button onClick={(e) => deleteUser(p.token, e)} className="p-3 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white rounded-xl transition-colors">✕</button>
                    </div>
                </div>
            ))}
          </div>
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
              <div className="bg-neutral-900 border border-white/10 p-6 rounded-3xl w-full max-w-sm shadow-2xl">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <button onClick={saveEdit} className="py-3 bg-green-500 hover:bg-green-400 text-black font-bold rounded-xl transition-colors">Save</button>
                  </div>
              </div>
          </div>
      )}
