...
export default function SalonBooking() {
  ...
  // --- 1. FETCH SALON DETAILS (NAME & MENU) ---
  useEffect(() => {
      const fetchDetails = async () => {
          try {
              ...
              if (res.ok) {
                  ...
                  setMenu(json.menu || []); // Load dynamic menu
              }
          } catch (e) { console.error("Failed to load salon details"); }
      };
      if (salonId) fetchDetails();
  }, [salonId]);
  ...
  if (!salonDetails) return (
      <div className="fixed inset-0 bg-black z-50 flex items-center justify-center">
          <p className="text-green-500 font-mono animate-pulse">LOADING SALON...</p>
      </div>
  );
      ...
      <div className="w-full max-w-md bg-neutral-900/50 backdrop-blur-xl border border-white/10 rounded-[2rem] shadow-2xl relative overflow-hidden">
         ...
         <div className="flex justify-between items-center p-6 border-b border-white/5">
            <div className="flex items-center gap-3">
                ...
                <div>
                    {/* 👇 Display Dynamic Salon Name */}
                    <h1 className="text-lg font-bold text-white leading-none">{salonDetails.name}</h1>
         ...
         {/* SERVICE MODALS NOW USE DYNAMIC MENU */}
         {view === "join" && (
             <div className="p-8">
         ...
         {/* (Keep Login/Signup forms as they were) */}
         {view === "login" && (
            <div className="p-8">
